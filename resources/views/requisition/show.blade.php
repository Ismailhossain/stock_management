                    ...
                    <table class="table table-striped custom-table mb-0 stockList" id="stockList" >
                        ...
                        @foreach($requisitions as $requisition)
                            <tr class="text-center">
                                ...
                                <td>
                                    @if($requisition->status == 0 && auth()->user()->type == 'employee')
                                    ...
                                    @elseif($requisition->status == 1 && auth()->user()->type == 'store_executive')
                                        @if($requisition->issue_status == 0)
                                        <a href="#0" class="btn btn-primary btn-sm" onclick="verifyIssue({{$requisition->id}})"  role="button">Issue Items</a>
                                        <form style="display: none" method="POST" id="verifyIssueItems-{{ $requisition->id }}"
                                              action="{{ route('requisition-issue-items', $requisition->id) }}">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                        @else
    ...
    <script>
        ...
        function verifyIssue(id){
            Swal.fire({
                ...
                if (result.value) {
                    event.preventDefault();
                    document.getElementById('verifyIssueItems-' + id).submit();
                    toastr.success('Requisition issued success.');
                }
            })
        }
