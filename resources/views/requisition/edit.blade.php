            ...
            <div class="col-sm-12 col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <!-- Form -->
                        <form action="{{ route('requisition-update',$requisition->id) }}" class="mt-2 form-validate" enctype="multipart/form-data" method="post">
                            @csrf
                            @method('put')
                            <div class="row">
                                <div class="col-md-12 col-12">
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="text"
                                               ...
                                               autofocus value="{{ isset($requisition->name) ? $requisition->name : '' }}" />
                                        @error('name')
...
@push('script')
    ...
    <script>
        ...
        $.post("{{ route('requisition-itemQty-ajax') }}", {
            id: '{{$requisition->id}}',
        }, function (response) {
            if (response.success == true){
                ...
                $.each(details, function (index, detail) {
                    tbl += '\n' +
                        ...
                        '     <td class="">\n' +
                        '         <span for="" class="social_icon">' + detail.item_name + '</span>\n' +
                        '         <input type="hidden" class="" id="item-'+detail.item_id+'"  name="store_item_id[]" value="' + detail.item_id + '">\n' +
                        '         <input type="hidden" class="" name="store_item_name[]" value="' + detail.item_name + '">\n' +
                        '     </td>\n' +
                        '     <td class="">\n' +
                        '         <span for="" class="social_link">' + detail.qty + '</span>\n' +
                        '          <input type="hidden" name="store_qty[]" value="' + detail.qty + '">\n' +
                        '     </td>\n' +
